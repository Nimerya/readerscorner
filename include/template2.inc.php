...
Class Template {
    ...
    /**
     * @param $numpages int numero di pagine
     * @return int
     */
    function setPageButtons($numpages){
        //se pn non è settata oppure è < 1 oppure è > numpages allora non va bene e lo metto a 1
        if (!isset($_REQUEST['pn']) || $_REQUEST['pn']<1 || $_REQUEST['pn']>$numpages) {
            $_REQUEST['pn'] = 1;
        }
        //mi metto da parte pn
        $pn=$_REQUEST['pn'];

        //setto valore pulsante della pagina corrente
        $this->setContent("currentPage",$pn);

        //setto valore pulsante dell'ultima pag
        $this->setContent("lastPage",$numpages);

        //se non ci sono pagine precedenti a quella attuale disabilito il pulsante per andare indietro
        if($pn==1)
            $this->setContent("disabled_prev", "disabled");

        //se ci sono ancora altre pagine oltre questa imposto il pulsante per andare avanti
        if($pn<$numpages)
            $this->setContent("nextPage", $pn+1);
        else //altrimenti lo disabilito
            $this->setContent("disabled_next", "disabled");

        //torno il numero di pagina attuale
        return $pn;
    }

    /**
     * @param $data array contenente i dati dalla uqery con cui riempire la pabella
     * @param string $aux opzionale, indica se questo è un report principale o ausiliario
     */
    function report($data, $aux=""){
    	global $system;
		$columns=count($data[0]); //conto il numero di colonne da i dati ricevuti
        $j=1; //contatore per stampare le $key
        foreach($data as $line) {
            $this->setContent("id",$line['id']); //setcontent id elemento per l'onclick
            $i=1; //contatore per stampare i $value
            foreach($line as $key=>$value){
				if($j<=$columns){
					$this->setContent("key", ucwords(str_replace('_', ' ', $key)));
					$j++;
				}
				if($i<=$columns){
					$this->setContent("value", htmlentities($value));
					$i++;
				}
            }
        }
        //se è un report main faccio il setcontent della lunghezza della pagina
        if($aux=""){
            $this->setContent("psize_a", $system->psize_admin);
        }
	}

    /**
	 * inietta un div con class="blank" nel tag specificato
     * @param $tag string il placeholder sul quale fare il setcontent
     * @param $text string il testo da inserire nel div
     */
	function addBlank($tag,$text){
    	$this->setContent($tag,"<div class=\"blank-page\">

        <h4 id=\"forms-horizontal\">{$text}</h4>
    </div>");
	}

    /**
     * @param $name string nome da mìinserire nell'input name=""
     * @param $label string
     * @param $required string indica se l'inpu è obbligatorio
     * @param $placeholder string
     * @param $pattern string
     * @param $type string
     * @param string $value il valor con cui precaricare l'input
     * @param string $style stile dell'input
     */
	function insertInput($name, $label, $required, $placeholder, $pattern, $type, $value="", $style=""){
    	...
    	switch($type){
			...
			case "password":
			case "number":
                $this->setContent("input", "<input type='{$type}' class=\"form-control\" style='{$style}' {$required} id='{$name}' name='{$name}' {$pattern} {$placeholder} {$value} />");
                break;
			case "textarea":
				$mce="";//disattibo TinyMCE sulla textarea "Style"
				if($name!="style"){
                    $mce="mceOn";
				}
                $this->setContent("input", "<textarea class=\"".$mce." form-control textarea-fitcontent\" style='{$style}' id='{$name}' name='{$name}' {$pattern} >$placeholder{$value}</textarea>");
                break;
			case "file":
                $this->setContent("input", "<input type=\"file\" class=\"form-control\" style='{$style}' {$required} id='{$name}' name='{$name}' {$value}>");
                break;
		}
        $this->setContent("name", $name);
        $this->setContent("label", $label);
    }
     ...
     * @param string $notes
     */
    function insertPic($name, $label, $value, $directory, $notes=""){
		if($notes!=""){//se ci sono note le visualizzo
			$notes="<p><strong>Notes:</strong></p>".$notes;
		}
        $this->setContent("input", "<img class=\"resized\" onerror=\"this.src='/readerscorner/images/error.png'\" src=\"/readerscorner/images/{$directory}/{$value}\"/>
                    <div class=\"notes\">
                        {$notes}
                    </div>");
        $this->setContent("name", $name);
    ...
    /**
	 * inserisce un input type hidden
     * @param $name string
     * @param $value string
     * @param $where string placeholder sul quale fare la setcontent
     */
    function insertHidden($name, $value, $where){
        $this->setContent($where, "<input type=\"hidden\" id='{$name}' name='{$name}' {$value}>");

    }

    /**
	 * fa il setcontent dei button su form edit/del del backoffice
     * @param $mode
     * @param string $text
     */
    function insertButtons($mode, $text="Insert"){
    	switch($mode){
			case "add":
				$this->setContent("button","<button type=\"submit\" class=\"btn btn-lg btn-success warning_1\">$text</button>");
				break;
			case "edit_del":
				$this->setContent("button","<button type=\"submit\" id=\"editbutton\" class=\"btn btn-lg btn-warning warning_11\">Edit</button>
													<div style=\"float: right;\">
														<button type=\"button\" id=\"to_hide\" class=\"btn btn-lg btn-danger\" onclick=\"document.getElementById('to_hide').style.display='none'; document.getElementById('to_show').style.display='inline'\">Delete</button>
														<button type=\"button\" id=\"to_show\" style=\"display: none;\" class=\"btn btn-lg btn-primary\" onclick=\"window.location='<[table]>.php?page=3&id='+document.getElementById('id').value\">Confirm</button>
													</div>");
				break;
            case "proceed":
                $this->setContent("button","<button type=\"submit\" class=\"btn btn-lg btn-info\">Proceed</button>");
                break;
			case "simpledel":
                $this->setContent("button","<button type=\"button\" id=\"to_hide\" class=\"btn btn-lg btn-danger\" onclick=\"document.getElementById('to_hide').style.display='none'; document.getElementById('to_show').style.display='inline'\">Delete</button>
											<button type=\"submit\" id=\"to_show\" style=\"display: none;\" class=\"btn btn-lg btn-primary\">Confirm</button>");
				break;
			case "del2":
                $this->setContent("button","<button type=\"button\" class=\"btn btn-lg btn-warning warning_11\" onclick=\"history.go(-1);\">Back</button>
													<div style=\"float: right;\">
														<button type=\"button\" id=\"to_hide\" class=\"btn btn-lg btn-danger\" onclick=\"document.getElementById('to_hide').style.display='none'; document.getElementById('to_show').style.display='inline'\">Unbind</button>
														<button type=\"submit\" id=\"to_show\" style=\"display: none;\" class=\"btn btn-lg btn-primary\">Confirm</button>
													</div>");
                break;
			default:
				Header("location: /readerscorner/admin/error.php?error=system");
				exit;
				break;
		}
	}
    ...
    /**
	 * inserisce una select con options date dall'array $options
     * @param $name string
     * @param $options array associativo [id]=>[valore] usato per costruire le options
     * @param $tag string indica il placeholder sul quale fare il setcontent
     * @param string $disabled
     * @param string $style
     * @param string $selected
     * @param string $mylabel
     */
	function insertSelect($name, $options, $tag, $disabled="", $style="", $selected="", $mylabel=""){

    	$op="";

    	//genero la select
    	foreach ($options as $id=>$n){
            $flag="";
    		if($selected==$id){
    			$flag="selected";
			}
    		$op.="<option {$flag} value=\"{$id}\">{$n}</option> ";
		}
		if($mylabel==""){//se non ho selezionato una label personalizzata
            $name_bello=ucfirst(str_replace("_"," ",$name));
            $label=$tag."_label";
            $for=$tag."_for";
            $this->setContent("{$for}", "{$name}");
            $this->setContent("{$label}", "Select a record from \"{$name_bello}\": ");
            $this->setContent("{$tag}", "<select {$disabled} class=\"form-control1\" style=\"width: 25%;{$style}\" name=\"{$name}\">{$op}</select>");

			}else{//genero la label
			$label="select_".$mylabel;
				$this->setContent($label, $mylabel);
				$this->setContent("{$tag}", "<select {$disabled} class=\"form-control1\" style=\"width: 25%;{$style}\" name=\"{$name}\">{$op}</select>");
			}
		}
    ////////////////////////////////////////FINE FUNZIONI NOSTRE/////////////////////////////////////
